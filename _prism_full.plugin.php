<?php
/**
 * Modified version of the Prism plugin for b2evolution
 * Changes:
 *
 * 1. No restriction on lanuguage you specify although edit screen still
 *    has original buttons.
 * 2. Add debug switch for non-minified versions for developers.
 *
 * https://github.com/davidnewcomb/prism_full_plugin
 *
 * ---
 ...
 * @package plugins
 */
class prism_full_plugin extends Plugin
{
	var $code = 'evo_prism';
	var $name = 'Prism (full)';
	var $priority = 27;
	...
	{
		$this->short_desc = T_( 'Display full computer code' ).' '.T_( '(Plugin not available in WYSIWYG mode)' );
		$this->long_desc = T_( 'Display dull computer code rendered by JavaScript plugin Prism.' ).' '.T_( '(Plugin not available in WYSIWYG mode)' );
	}
...
// 		if( ! in_array( $lang, array( 'java', 'php', 'css', 'javascript', 'sql', 'markup', 'apacheconf' ) ) )
...
// 			if( empty( $lang ) || ! in_array( $lang_match[1], array( 'php', 'java', 'css', 'javascript', 'sql', 'markup', 'apacheconf' ) ) )
